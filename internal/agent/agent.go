...
const (
	...
	EventRoundStart     EventType = "round_start"     // new agent round; StatusMsgIdx carries the status message index
)
...
type Event struct {
	...
	// EventRoundStart: index of the newly-added RoleStatus message for this round.
...
func (a *Agent) loop(ctx context.Context, sess *session.Session, ch chan<- Event) {
	...
	for {
		...
		statusIdx := sess.AddMessage(session.Message{Role: session.RoleStatus})
		select {
		case ch <- Event{Type: EventRoundStart, StatusMsgIdx: statusIdx}:
		case <-ctx.Done():
			return
		}
		...
		// For mixed rounds (text + tools), redirect status events to a new post-text
		// status message so ops appear after the text box, not before it.
		if content != "" {
			postStatusIdx := sess.AddMessage(session.Message{Role: session.RoleStatus})
			select {
			case ch <- Event{Type: EventRoundStart, StatusMsgIdx: postStatusIdx}:
			case <-ctx.Done():
				return
			}
		}
